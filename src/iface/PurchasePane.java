...
public class PurchasePane extends ActivityPane {

	protected PurchasePane() {
		super();
		...
		new Thread(()-> {
			...
			// Getting all stock supplies data (by communication with the second tier) and updating the UI:
			ArrayList<Stock> supplies = AppLogic.getAppData().getStockSupplies();

			// Filling in the table with the correct data:
			for(int i = 0; i < supplies.size(); i++){
		    	String stockName = supplies.get(i).getStockName();
		    	int amount = supplies.get(i).getAmount();
		    	int price = supplies.get(i).getPrice();
		    	String description = supplies.get(i).getDescription();
			...
			Label stockMarketTitle = new Label("Stock Market's Supplies");
			...
			Label stockMarketTip = new Label("Tip: The following table should help you make a better purchase decision."
					+ "\nYet, you could always demand something that isn't currently supplied by the stock market.");
			...
			Label purchaseTitle = new Label("Make A Purchase/Demand");
			purchaseTitle.setMinHeight(40);
			purchaseTitle.setFont(new Font("Arial", 16));
			purchaseTitle.setUnderline(true);
			...
			Button callToActionButton = new Button("Place Bid");
			...
			primaryPane.getChildren().addAll(purchaseTitle, greedNBottonPane);
			    		...
			    		if(Integer.parseInt(amount.getText()) > 0 && Integer.parseInt(price.getText()) > 0) {
				    		...
				    		new Thread(()-> {
					    		...
					    		TransactionEvents.eventType result = AppLogic.getAppData().makeTransaction(
					    				...
					    				TransactionEvents.eventType.BID_ATTEMPT);

					    		Platform.runLater(()-> {

					    			removeFeedback();

					    			if(result == TransactionEvents.eventType.NOT_ENOUGH_MONEY) {
					    				displayFeedback("Sorry, the ransaction could not be made."
					    						+ "\nThere is not enough money in your bank account.");
					    			}
